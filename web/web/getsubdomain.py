...
class SubDomain:
    ...
    def chaxunla(self):
        ...
        sub = []
        res = requests.get('http://api.chaxun.la/toolsAPI/getDomain/?k={}&action=moreson'.format(self.url),
        ...
        if "status" in res and res['status'] == 3:
            ...
            while True:
                page = 1
                res = requests.get(
                    'http://api.chaxun.la/toolsAPI/getDomain/?k={}&action=moreson&page={}'.format(self.url, page),
                    headers=self.headers)
                res = res.json()
                if (res['status'] == -2):
                    break
    ...
    def chinaz(self):
        ...
        if len(p.findall(res.text)) >= 1:
            ...
            for i in range(1, page + 1):
                res = requests.get('http://tool.chinaz.com/subdomain?domain={}&page={}'.format(self.url, i),
                                   headers=self.headers)
    ...
    def fofa_api(self):
        ...
        res = requests.get(
            'https://fofa.so/api/v1/search/all?email={}&key={}&qbase64={}&size={}'.format(email, key, s, size))
        res = res.json()
